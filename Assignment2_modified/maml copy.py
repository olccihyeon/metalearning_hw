...
class MAML:
    ...
    def _outer_loop(self, task_batch, train=None):
        ...
        for task in task_batch:
            ...
            minibatch_loss = []
            with tf.GradientTape() as tape:
                for _, (x_query, y_query) in enumerate(query):
                    logits = self.model(x_query, training=True)
                    minibatch_loss.append(loss_fn(y_query, logits))

                outerloss = tf.reduce_mean(minibatch_loss)
                tf.nest.map_structure(lambda x, y: x.assign(y), self.model.trainable_weights, theta)
                grads = tape.gradient(outerloss, self.model.trainable_weights)
                self._optimizer.apply_gradients(zip(grads, self.model.trainable_weights))
                outer_loss_batch.append(outerloss)
