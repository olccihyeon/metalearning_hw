...
class DataLoader(object):
    ...
    def generate_task_list(self, n_tasks:int=100, n_way:int=0, n_query:int=0, n_support:int=0):
        ...
        for i in range(n_tasks):
            ...
            for j in range(n_way):
                listindex  = list(range(20))
               
                label=np.random.choice(list(self.data.keys()))
                supportlist = np.random.choice(listindex,n_support)
                labeldict[label] = supportlist
                querylist = np.random.choice(list(set(listindex)- set(supportlist)))
                np.append(labeldict[label],querylist)
        
            task_list.append(labeldict)
    ...
    def data_generator(self, task_idx=0):       
        ...
        for index, (key, value) in enumerate(task.items()):
            for i in range(self.n_support):
                support[index,i,:,:] += self.data[key][i]
